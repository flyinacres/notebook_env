...
*What to do:* you'll need to switch to a Python version that matches what the message says the notebook needs, then run the notebook again from the top. If you're not sure how to do that on whatever platform you're using (your own computer, Kaggle, Colab, etc.), that's worth a quick search for "how to change Python version" plus the platform's name.
...
*What to do:* usually nothing. If something else fails further down in a way that doesn't match anything else on this page, come back and consider this the likely reason.
...
*What to do:* if you keep seeing "Retrying" and nothing progresses, check your internet connection, then run that cell again.

**"Setup complete! Environment ready."**

Good news, everything installed correctly.

*What to do:* nothing. Continue running the rest of the notebook.

**"Setup failed while installing pinned dependencies."**

Something didn't install correctly. The message right after this one will suggest it's about a graphics-card-specific version mismatch, but that suggestion isn't always right, it's shown even when that's not the actual cause.

*What to do:* scroll up, above this failure message, to the actual error text from the install step. That's where the real reason is, usually one of: no internet connection, a package name that's changed or no longer exists, or a permissions/disk-space problem on your machine. A graphics-card mismatch is only one possibility among several, don't assume that's it just because the message mentions it. If you genuinely can't tell what went wrong from the text above, the person who gave you this notebook is your best next step, they'll know what it's supposed to need.
...
*What to do:* if your computer doesn't have one, the notebook will most likely still run, just more slowly. Not a problem to fix, just something to expect.
...
*This page covers v35 of this notebook's setup cells. If your notebook shows a different version number in its first two cells, some wording above might not match exactly.*
