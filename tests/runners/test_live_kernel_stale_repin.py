...
def main() -> int:
    ...
    # Fixture generation/read-back is wrapped so a failure here (bad tool
    # invocation, malformed output) still cleans up the temp fixture file
    # from the actual repo directory (bind-mounted, not container-ephemeral)
    # instead of leaking it on every failed run.
    try:
