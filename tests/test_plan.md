...
Distinct from — and a partial answer to — the "full reproducibility can't be made airtight" limitation noted in Phase 4's update below. Every install-engine fixture built in Phase 7 depends on real PyPI continuing to serve specific old wheels (humanize==4.16.0, tabulate==0.9.0, numpy==1.23.5) indefinitely — usually fine, but an external dependency the suite doesn't control (a maintainer dropping old wheel builds, a yanked release). Building and owning a tiny test-only package removes this risk entirely for the narrower, airtight-able claim: "does our pin-and-install mechanism deterministically reproduce known behavior," as opposed to the broader, not-fully-airtight-able claim "does this work against arbitrary real packages" (which Phase 4's hand-triaged sampling remains the right, imperfect answer for).

Mechanics: a small owned package (e.g. notebook_env_test_fixture), version baked into actual code (a function returning a hardcoded literal, not just metadata) so a functional call proves that version's code genuinely ran, cross-checked against importlib.metadata.version() to catch a mismatched build. Ships as both wheel and sdist artifacts checked into the fixtures directory — plain version-pin scenarios need only wheels, but the timeout and build-failure scenarios below require an sdist with a custom build hook, since a wheel install is just file unpacking with no code execution to slow down or fail. Installed via --find-links <dir> --no-index (not --find-links alone), so pip is physically prevented from ever reaching the real index regardless of package-name collision risk. Runs inside a disposable python:3.11-slim container, same as Phase 7's fixtures, for the same reason: real installs mutate whatever environment executes them.

This is a mini-suite, not one fixture — each of the following exercises a genuinely different code path, confirmed against current source:

Base pin-and-verify: install v1.0.0, re-pin to v2.0.0, confirm the mechanism actually swaps versions (functional + metadata check on both).
-f/--find-links tracked install: confirms per-occurrence pip flags are correctly parsed, preserved verbatim, and reach the actual generated install command (traced to dep.flags in generate_production_blueprint) — this is the supported, tracked local-install path.
Bare local file path as specifier (e.g. pip install ./wheels/pkg-1.0.0-py3-none-any.whl, no flag): confirmed this matches VCS_OR_PATH_PREFIXES and is skipped entirely, same bucket as git+https://... — a negative-shaped test confirming this documented scope boundary explicitly, not a bug to fix.
Local-version hardware-tag suffix (1.0.0+customhw, valid PEP 440): hits the local_builds_section warning path in generate_production_blueprint — a markdown-generation code path, not the install engine, and currently untestable by anything else (5f fakes the hardware-detection call itself but can't produce a real PEP 440 local-version string).
Deliberately slow-installing version (sdist, build hook sleeps): exercises the install engine's 120-second per-package subprocess timeout branch (subprocess.TimeoutExpired handling) — exists in code, currently untested by anything.
Genuine build-time failure (sdist, build hook raises/exits nonzero): distinguishes "no matching distribution" (already covered by Phase 7) from "found it, but installing it failed" — may hit a different branch in the engine's error handling.

Side finding surfaced while scoping this, tracked separately rather than folded into 5k's pass/fail criteria: harvest_index_urls_from_sources collapses -f/--find-links into the same bucket as --extra-index-url and always re-emits it as --extra-index-url in the markdown "Extra Index URLs Harvested" summary and --universal manifest text. Doesn't affect the real per-notebook install (that path preserves the original flag correctly), but the generated documentation could describe a local find-links directory using the wrong pip syntax. Worth its own small test, possibly a fix — not a blocker for this phase.
